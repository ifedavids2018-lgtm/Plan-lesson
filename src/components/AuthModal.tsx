...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onShowToast }) => {
  const { login, register, loginWithGoogle, loginDemoUser } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        onShowToast('Teacher account registered with Firebase Auth.', 'success');
      } else {
      ...
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl border border-slate-200">
        ...
        <div className="mt-4">
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-xl border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-xs transition shadow-xs disabled:opacity-60"
          >
        ...
        <div className="my-3 text-center text-[11px] text-slate-400 relative">
          <span className="bg-white px-2 relative z-10 font-medium">or email & password</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2.5 rounded-xl bg-blue-700 text-white font-bold text-xs hover:bg-blue-800 transition shadow-sm mt-2 disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {loading ? (
              'Authenticating...'
            ) : isRegister ? (
